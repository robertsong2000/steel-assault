# Cursor Automation — 半小时 TODO 工人（TDD）

把下面「Automation Prompt」整段粘贴到 [cursor.com/automations/new](https://cursor.com/automations/new)。

> **重要**：Cloud Agent 按用量计费；`*/30 * * * *` 一天约 48 次。建议先启用确认流程跑通，再视账单调整为每小时 `0 * * * *`。

## 仪表盘配置

| 字段 | 值 |
| --- | --- |
| Name | `STEEL ASSAULT — TODO TDD Worker` |
| Trigger | Scheduled → cron `*/30 * * * *` |
| Repository | `robertsong2000/steel-assault`（**必须挂仓库**，否则无法改代码/开 PR） |
| Branch | `main` |
| Tools | Pull request creation ✅；Memories ✅（用于记住上次完成的队列 id） |
| Model | 默认或你常用的强模型 |

## Automation Prompt（复制从此处开始）
...
| 文件 | 作用 |
| --- | --- |
| `AUTOMATION_QUEUE.md` | 机器可读任务队列（本自动化的唯一选源） |
| `AGENTS.md` | 所有 Cloud Agent 的 TDD / 回归约定 |
| `.cursor/automations/todo-tdd-runner.prompt.md` | 上述 prompt 的仓库副本，方便改完后同步到仪表盘 |
| `package.json` → `npm run check` | 提交前门禁 |
